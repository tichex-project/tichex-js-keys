...
const hdPathAtom = `m/44'/118'/0'/0/0` // key controlling BTSG allocation
...
export function getNewWalletFromSeed(mnemonic: string): Wallet {
  ...
  const tichexAddress = getTichexAddress(publicKey)
  return {
...
export function getTichexAddress(publicKey: Buffer): string {
  const message = CryptoJS.enc.Hex.parse(publicKey.toString('hex'))
  const address = CryptoJS.RIPEMD160(CryptoJS.SHA256(message) as any).toString()
  const tichexAddress = bech32ify(address, `tichex`)

  return tichexAddress
}
...
function deriveKeypair(masterKey: bip32.BIP32): KeyPair {
  const tichexHD = masterKey.derivePath(hdPathAtom)
  const privateKey = tichexHD.privateKey
